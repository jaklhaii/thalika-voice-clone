...
def handle_message(update: dict):
    ...
    if st.get("step") == "waiting_audio":
        ...
        if audio:
            ...
            if not fid:
                send_message(chat_id, "⚠️ Failed to read the audio file — please resend.", reply_markup=kb_main())
                return
            send_message(chat_id, "⏳ Downloading your voice sample...")
            r = api("getFile", {"file_id": fid})
            fpath = (r.get("result") or {}).get("file_path", "")
            if not fpath:
                send_message(chat_id, "⚠️ Failed to download the audio — please resend.", reply_markup=kb_main())
                return
            ...
            try:
                ...
                send_message(chat_id, "⚠️ Download failed — please resend.", reply_markup=kb_main())
                return
            if len(data) > MAX_REF_MB * 1024 * 1024:
                send_message(chat_id, f"⚠️ File too large (max {MAX_REF_MB} MB). Send a shorter sample.", reply_markup=kb_main())
                return
            ...
            if save_voice(uid, name, base64.b64encode(data).decode()):
                ...
                send_message(chat_id, "⚠️ Save failed — please try again.", reply_markup=kb_main())
            _state[chat_id] = {}
            return
        else:
            send_message(
    ...
    if st.get("step") == "waiting_text":
        ...
        if not voice:
            send_message(chat_id, "⚠️ Reference voice missing — please add it again via <b>My Voices</b>.", reply_markup=kb_main())
            _state[chat_id] = {}
            return
        ...
        if err:
            send_message(chat_id, f"⚠️ Generation failed:\n{err}", reply_markup=kb_main())
            _state[chat_id] = {}
            return
        send_voice(chat_id, wav, caption=f"{BOT_NAME} — voice: {voice[2]}")
        _state[chat_id] = {}
        return
    ...
    if st.get("step") == "waiting_adduser":
        ...
        if user_allowed(target):
            send_message(chat_id, "ℹ️ That user already has access.", reply_markup=kb_admin_users())
            _state[chat_id] = {}
            return
        add_allowed(target, "", uid)
        send_message(chat_id, f"✅ User <code>{target}</code> added to allowed list.", reply_markup=kb_admin_users())
        _state[chat_id] = {}
        return
    ...
    if st.get("step") == "waiting_remuser":
        ...
        send_message(chat_id, f"✅ User <code>{target}</code> removed.", reply_markup=kb_admin_users())
        _state[chat_id] = {}
        return

    # ---------- COMMANDS ----------
    if cmd in ("/start", "/help"):
        send_message(chat_id, text_for(uid), reply_markup=kb_main())
        return
    ...
    if cmd == "/voice" and args.strip():
        ...
        if not voices:
            send_message(chat_id, "⚠️ No voices saved yet.\n<b>My Voices</b> → <b>Add Voice</b> first.", reply_markup=kb_main())
            return
        voice = get_voice(uid, voices[0][1])
        if not voice:
            send_message(chat_id, "⚠️ Reference voice missing.", reply_markup=kb_main())
            return
    ...
    if cmd in ("/voice",) and not args.strip():
        send_message(chat_id, "Send <b>/voice your text here</b> after selecting a voice in <b>My Voices</b>.", reply_markup=kb_main())
        return
...
def handle_callback(update: dict):
    ...
    if data == "voice:list":
        ...
        voices = list_voices(uid)
        if not voices:
            send_message(chat_id, " No voices saved yet.\n<b>Add Voice</b> → send a voice message.", reply_markup=kb_voices())
            return
        send_message(chat_id, "<b>Select a voice to use:</b>", reply_markup=kb_voice_list(voices))
        return
    if data.startswith("voice:use:"):
        ...
        if not v:
            send_message(chat_id, "⚠️ Voice not found — please add it again.", reply_markup=kb_voices())
            return
    ...
    if data == "gen:text":
        ...
        voices = list_voices(uid)
        if not voices:
            send_message(chat_id, "⚠️ No voices saved yet — <b>Add Voice</b> first.", reply_markup=kb_voices())
            return
    ...
    if data == "admin:users":
        api("answerCallbackQuery", {"callback_query_id": cb["id"]})
        send_message(chat_id, "<b>User Access Management</b>", reply_markup=kb_admin_users())
        return
    ...
    if data == "admin:remuser":
        api("answerCallbackQuery", {"callback_query_id": cb["id"]})
        users = list_allowed()
        send_message(chat_id, "<b>Select user to remove:</b>\n(or send/reply with their ID)", reply_markup=kb_admin_remove(users))
